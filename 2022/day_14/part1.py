...
grid = [["." for _ in range(offset_x, int(max_x) + 2)] for _ in range(offset_y, int(max_y) + 2)]
    ...
    for x, y in reversed(edges):
        if old_x - x > 0:
            for nx in range(x, old_x + 1):
                grid[y - offset_y][nx - offset_x] = "#"
        elif old_x - x < 0:
            for nx in range(old_x, x + 1):
                grid[y - offset_y][nx - offset_x] = "#"
        elif old_y - y > 0:
            for ny in range(y, old_y + 1):
                grid[ny - offset_y][x - offset_x] = "#"
        elif old_y - y < 0:
...
def spawn_rock(x: int, y: int) -> bool:
    while True:
        ...
        if grid[y - offset_y + 1][x - offset_x] == ".":
            y += 1
        elif grid[y - offset_y + 1][x - offset_x - 1] == ".":
            y += 1
            x -= 1
        elif grid[y - offset_y + 1][x - offset_x + 1] == ".":
            y += 1
            ...
            grid[y - offset_y][x - offset_x] = "o"
            return True

count = 0
while spawn_rock(500, 0):
    count += 1
print(count)
